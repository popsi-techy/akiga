...
import type { WorkflowNode, WorkflowBranch, ConditionGroup } from '@/data/automation-types';
import { addElseIf, removeBranch, patchBranch } from '@/lib/workflow-tree';
    ...
    <div className="space-y-6">
      ...
      {editingBranch && (
        <ConditionBuilderDrawer
          ...
          subtitle="Identities matching these conditions follow this path."
          group={editingBranch.condition ?? emptyConditionGroup()}
          onApply={(group: ConditionGroup) => setBranches(patchBranch(branches, editingBranch.id, { condition: group }))}
        />
      )}
    </div>
  );
...
export default WfConditionalConfig;
