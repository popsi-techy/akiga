...
/**
 * The scope and disclaimer that frames every evidence package for a framework: what the
 * package is, what it is not (a certification — none exists), who stays responsible, and the
 * boundary of the evidence.
 *
 * It lives here, not at a call site, because two surfaces state the same fact and it must
 * read identically in both: the framework detail page states it as provenance the reader sees
 * before trusting the matrix, and the seal modal states it as what sealing commits you to.
 * A disclaimer that is worded two ways is a disclaimer that means neither.
 *
 * `short` is the one-line summary for a collapsed affordance; `full` is the complete statement.
 */
export function scopeOfEvidence(frameworkName: string): { short: string; full: string } {
  return {
    short: 'Not a certification of compliance · identity governance scope only',
    full:
      `This package contains identity and access governance evidence generated by this platform to ` +
      `support the organisation's own assessment against applicable requirements of the ${frameworkName}. ` +
      `It is not a certification of compliance, and no such certification scheme exists. The organisation ` +
      `remains responsible for assessing control effectiveness, determining its maturity level, addressing ` +
      `findings and obtaining required approvals. Evidence is limited to the identity governance scope ` +
      `described in this package and to the systems onboarded to the platform.`,
  };
}
